...
class Chip extends BaseCommand {
    ...
    async execute(ctx, chip) {
        ...
        if (allChips.includes(chip.toLowerCase())) {
            ...
            if (meta.author)
                embed.addField('Author', meta.author, false);
            if (meta.link || meta.url)
                embed.setURL(meta.link || meta.url);
            if (meta.thumbnail)
                embed.setThumbnail(meta.thumbnail);
            if (meta.color || meta.colour)
                embed.setColor(meta.color || meta.colour);
            return ctx.send(embed);
        }
        else {
